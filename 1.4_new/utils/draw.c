...
void ColorConden(HWND hWnd, HDC hdc, float** A, int** Acoords, float** Astrong, char** Anames, int N){
    int r = 30, dtx = 8;
    HPEN Pen;
    for(int i = 0; i < N; i++){
        Pen = CreatePen(PS_SOLID, 2, RGB(10*i, 0, 20*i)); 
        SelectObject(hdc, Pen);
        for(int l = 0; l < N; l++){
            if(Astrong[i][l]){
                MoveToEx(hdc, Acoords[l][0], Acoords[l][1], NULL);
                Ellipse(hdc, Acoords[l][0]-r+(i+1)*2, Acoords[l][1]-r+(i+1)*2, Acoords[l][0]+r-(i+1)*2, Acoords[l][1]+r-(i+1)*2);
