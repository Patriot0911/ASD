...
void ColorPeaks(HWND hWnd, HDC hdc, int N, char** Anames, int** Acoords, int count){
    ...
    for(int i = 0; i < count; i++){
        Pen = CreatePen(PS_SOLID, 4, RGB(50*(i+1), 50*(i+1), 20*(i+1))); 
        SelectObject(hdc, Pen);
        for(int l = 0; l < N; l++){
            if(components[l] == i){
                MoveToEx(hdc, Acoords[l][0], Acoords[l][1], NULL);
                Ellipse(hdc, Acoords[l][0]-sradius+(i+1)*4, Acoords[l][1]-sradius+(i+1)*4, Acoords[l][0]+sradius-(i+1)*4, Acoords[l][1]+sradius-(i+1)*4);
            }
        }
    }
}
